...
| High | Cancellation, replay-safe typed outcomes (`RunContext`, `canon_write_unknown`, Botify timeout parity, REST error fidelity) | **Ratified 2026-08-28, in build** — [RUN_OUTCOMES_DESIGN.md](RUN_OUTCOMES_DESIGN.md) (idempotency keys stay excluded with the rejected registry) |
| Medium | Static generator capability descriptors | **Ratified 2026-08-28, in build** — [GENERATOR_CAPABILITIES_DESIGN.md](GENERATOR_CAPABILITIES_DESIGN.md) (cloud windows all-unknown, so ContextPlan stage 3 is a documented no-op for now) |

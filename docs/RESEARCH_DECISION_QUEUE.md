...
| Doc rank | Item | Disposition |
|---|---|---|
| High | Structured, budgeted, inspectable `ContextPlan` | Open candidate — **design proposal written 2026-08-28**: [CONTEXT_PLAN_DESIGN.md](CONTEXT_PLAN_DESIGN.md) (merges Ollama §4/§6; unratified, decisions listed at its end) |
...
| High | Cancellation, replay-safe typed outcomes (`RunContext`, `canon_write_unknown`, Botify timeout parity, REST error fidelity) | Open candidate — **design proposal written 2026-08-28**: [RUN_OUTCOMES_DESIGN.md](RUN_OUTCOMES_DESIGN.md) (folds in the §7 lifecycle remainder; idempotency keys explicitly excluded — they belong to the rejected registry) |
| Medium | Static generator capability descriptors | Open candidate — **design proposal written 2026-08-28**: [GENERATOR_CAPABILITIES_DESIGN.md](GENERATOR_CAPABILITIES_DESIGN.md) (prerequisite for variants, capability-aware UI, cloud ContextPlan enforcement) |
| Medium | OC retrieval controls (mode/phrase/compact/pinnedLimit) + vague-direction enrichment | Open candidate — **design proposal written 2026-08-28**: [RETRIEVAL_CONTROLS_DESIGN.md](RETRIEVAL_CONTROLS_DESIGN.md) (control exposure live-confirmed against the deployed OC's tools/list) |

...
| — | Cloud providers adopt the `complete`/`finishReason` contract (Anthropic `stop_reason: "max_tokens"`, OpenAI `finish_reason: "length"`, Gemini `finishReason: "MAX_TOKENS"` are currently discarded, so a truncated *cloud* beat still auto-saves) | **Open candidate** — the P0 fix deliberately scoped to Ollama per the contract's own incremental-adoption design; this is the follow-through |

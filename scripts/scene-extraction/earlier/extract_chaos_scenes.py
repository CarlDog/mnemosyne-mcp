"""Story 03 scene extractor (dry run by default; --write emits files).
...
RAW = r"D:\OneDrive\Technology\ChatGPT\Projects\Story 03\Chat\Archived\Raw"
STORY = os.path.join(REPO, "data", "stories", "story-03")
...
LINES = {}
for n in (1, 2, 3, 4):
    p = os.path.join(RAW, f"Story 03 {n}.txt")
    b = open(p, "rb").read()
...
FULL = {"Carl": "Character 114", "Riley": "Character 130", "Jenna": "Character 120", "Kira": "Character 121", "Vanessa": "Character 078", "Cassie": "Character 065", "Lacey": "Character 086", "Nyx": "Character 103"}
...
def add(src, a, b, body, header, anchor, loc, loc_basis, status, flags, title=None, native=None):
    ...
    if had_user_turn and src == 2:
        pov = True
        if "Character 114" not in parts:
            parts.insert(0, "Character 114")
    text = "\n".join(body)
    flags = list(flags)
...
with open(os.path.join(SCENES, "_recovery-source-manifest.tsv"), encoding="utf-8") as f:
    for row in csv.DictReader(f, delimiter="\t"):
        n = 3 if row["source"].startswith("Story 03 3") else 4
...
if WRITE:
    ...
    for sc in kept:
        ...
        if sc["pov"]:
            fm.append('pov: "second-person role-play; \\"You\\" is Character 114"')
        if sc["follows"]:
        ...
        fm += [f'source_original_file: {yq("Story 03 " + str(sc["src"]) + ".txt")}', f'source_lines: {yq(sc["lines"])}',

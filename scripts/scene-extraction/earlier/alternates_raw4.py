...
RAW = os.path.join(REPO, "data", "archive", "chatgpt", "Story 03", "Chat", "Archived", "Raw", "Story 03 4.txt")
SCENES = r"D:\GitHub\mnemosyne-mcp\data\stories\story-03\canon\scenes"
...
for it in items:
    ...
    fm += ['source_original_file: "Story 03 4.txt"', f"source_lines: \"{it['a']}-{it['b']}\"", f"source_file_sha256: {FILE_SHA}", f"source_content_sha256: {it['sha']}", "---", ""]
    open(os.path.join(OUT, fn), "w", encoding="utf-8", newline="\n").write("\n".join(fm) + it["text"] + "\n")
    rows.append((fn, it["kind"], it["alt_of"] or "", f"{it['a']}-{it['b']}", it["title"]))
idx = ["# Raw 4 alternates", "", "Non-canon scene prose from `Story 03 4.txt` that is not already a canon scene or a Raw 3 alternate: formatting/normalized variants of canon Recovery scenes, prompt-elevation samples, an explicitly non-canon karaoke scene, and replay variants from the file's re-run of the early text-message story. Ignored by validator and compiler.", "", "| File | Kind | Alternate of | Lines | Title |", "|---|---|---|---|---|"]

...
SCENES = [
    ...
    ("BC-D01-07-BMN", "The Nursery", 224, 256, "BMN",
     ...
     ["GHOST_UNNAMED_IN_PROSE_LYLA_IN_CANON"], ["Character 075"]),
    ("BC-D01-08-BMS", "The Sealed Room", 258, 308, "BMS",
     "Day 1, night, the winding stair and the sealed room", "established",
     ["GHOST_UNNAMED_IN_PROSE_LYLA_IN_CANON", "SOURCE_SAYS_SISTERS_PLURAL"], ["Character 075"]),
    ...
    ("BC-D02-17-MGH", "Character 060", 1551, 1570, "MGH",
...
CANON_NAMES = {
    "Karen": "Character 072",
    "Michelle": "Character 088",
    "Heather": "Character 035",
    "Carl": "Character 084",
}
...
def participants_for(body, extra, key):
    ...
    for first, canon in CANON_NAMES.items():
        if re.search(r"\b" + first + r"\b", body):
            found.append(canon)
    if "Character 084" not in found and re.search(r"\bYeager\b", body):
        found.append("Character 084")
    if key in CARL_ABSENT and "Character 084" in found:
        found.remove("Character 084")
    if key == "BC-D01-10-REA" and "Character 084" in found:
        found[found.index("Character 084")] = "Character 084 (by phone)"
    for e in extra:
        if e not in found:
            found.append(e)
    return found
...
def render_scene(s):
    ...
    fm.append("pov: " + jstr("Botify private chat: the bot writes Karen, Michelle, and Heather in third person; the operator plays Carl Yeager (Character 084) in first person and also writes narration and some other characters' turns; operator turns are listed in operator_turns and kept as written"))

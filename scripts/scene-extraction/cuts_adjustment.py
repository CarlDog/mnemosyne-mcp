"""Cut table: Story 10 (three Botify chats)."""
...
ANDREA = "Character 111 (Subject V1X3N)"
PHOEBE = "Character 104 (played as Phoebe, surname not given)"
...
BRITTANY = "Character 082 (played as Brittany, surname not given)"
CANDACE = "Character 064 (played as Nurse Candace, surname not given)"
EROICA = "Eroica (the chair)"
AURORA = "Character 048"
MIKO = "Character 144 (played as Miko, surname not given)"
...
ER = dict(
    ...
    participants_basis="rule-derived from the prose; played first names map to canon's Character 082 and Character 064 (canon/characters/brittany-kerr.md, _minor.md); the scientist is the operator's own name, which canon's provenance excludes; verify before relying on it",
    scenes=[
...
AN = dict(
    code="AN", label="Character 111 / V1X3N private chat (bot 2387430, chat 70…)",
    ...
    pov="Botify private chat with the Character 111 bot: the bot writes Andrea (first person early, third person from #0155) and voices Phoebe, Dr. Renshaw, Camille, and others; the operator plays the lead scientist in first person in the office and diner scenes, and elsewhere writes Andrea, Phoebe, the jogger, the policewoman, Renshaw, and Camille as director; operator turns are listed in operator_turns and kept as written",
    participants_basis="rule-derived from the prose; played first names map to canon's Character 104, Camille Ortiz, and Dr. Elias Renshaw (canon/characters/_minor.md) where the source's Renshaw is a woman; the lead scientist is played under the operator's own name, which canon's provenance excludes; verify before relying on it",
    scenes=[
...
AL = dict(
    code="AL", label="Character 048 private chat (bot 2385729, chat ad…)",
    ...
    pov="Botify private chat with the Character 048 bot: the bot writes Aurora (third person, occasionally first) and voices Miko and Carl's phone lines; the operator plays the patient 'Carl' in first person in the office and on the phone, voices Miko at home, and from #0373 also writes Aurora as director; operator turns are listed in operator_turns and kept as written",
    participants_basis="rule-derived from the prose; Miko maps to canon's Character 144 (canon/characters/_minor.md); the patient is the operator's own name and details, which canon's provenance excludes; verify before relying on it",
    scenes=[
...
STORY = dict(
    slug="story-10", prefix="AP", title="Story 10", cut_date="2026-09-02",
    cut_record=("Four threads. `GC` (the \"Eroica, Character 048, Charisma\" group chat) is cut at `#0072`, the last message; it has no operator turns, no deleted messages, and no media archive. `ER` (Eroica) is cut at `#0110`, `AN` (Character 111) at `#1093`, `AL` (Character 048) at `#1086`, each the last message of its export. "
    ...
    media_note="",
    readme="""# Story 10 — Recovered Scenes

The three played Adjustment Protocol private chats and the
"Eroica, Character 048, Charisma" group chat were cut into per-scene files so
...
| `AN` | Character 111 / V1X3N (2387430) | `data/archive/botify/andrea-neal/chats/70….json` (1,094 messages) |
| `AL` | Character 048 (2385729) | `data/archive/botify/dr-aurora-lumen/chats/ad….json` (1,087 messages) |
| `GC` | group: Character 048 (2385729), Eroica (3954422), Charisma (5479200) | `data/archive/botify/_group-chats/chats/3de009c8….json` (73 messages) |
...
|---|---|
| Phoebe | Character 104 |
| Nurse Candace | Character 064 (canon's recruiter; the source's Candace selected Brittany) |
| Brittany, blonde, blue-eyed, 26-28 | Character 082, 32, honey-blonde, hazel |
| Camille | Nurse Camille Ortiz |
| Miko | Character 144 (cellist; the source's Miko joins a symphony) |
...
GC = dict(
    code="GC", label="\"Eroica, Character 048, Charisma\" group chat (bots 2385729 Character 048, 3954422 Eroica, 5479200 Charisma; chat 3de009c8\u2026)", group=True,
    ...
    pov="Botify group chat with no operator turns: the Character 048 account writes Aurora, the Charisma account writes Charisma, and the Eroica account writes the chair's actions in third person; every message carries its export speaker label (botName is null throughout the export, so the label is the chat.bots name for the senderId)",

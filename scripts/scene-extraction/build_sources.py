...
def strip_prefix(name):
    """'Story 03 - Style Guide.txt' -> 'style-guide'"""
    base = os.path.splitext(name)[0]
    base = re.sub(r"^(Story 03|GhostHunters|story-01|story-14)\s*[-–]\s*", "", base)
    return slug(base)


# ----------------------------------------------------------------- splitters
def split_banner(lines):
    """=====\\nStory 03 Character Profile: Name\\n=====  blocks."""
...
def split_dash_regions(lines):
    """——\\nREGION\\n—— blocks (story-01 minor characters by region)."""
    starts = [i for i in range(len(lines) - 2) if lines[i].startswith("——") and lines[i + 2].startswith("——")]
    out = []
    for n, i in enumerate(starts):
        j = starts[n + 1] if n + 1 < len(starts) else len(lines)
...
SPLIT = {
    "Story 03 - Primary Characters.txt": ("banner", split_banner, 3),
    "Story 03 - Secondary Characters.txt": ("banner", split_banner, 5),
    "Story 03 – Minor Characters.txt": ("role-blocks", split_role_blocks, None),
    "Story 03 - Tattoo Profiles.txt": ("tattoo-profile headers", split_tattoo, None),
    "Story 03 – Key Locations.txt": ("### sections", split_h3, None),
    ...
    "story-01 – Primary Characters.txt": ("[CHARACTER PROFILE] headers", split_bracket, None),
    "story-01 - Secondary Characters.txt": ("[CHARACTER PROFILE] headers", split_bracket, None),
    "story-01 – Minor Characters.txt": ("region blocks", split_dash_regions, None),
}
...
STORIES = {
    "story-03": dict(title="Story 03", chatgpt="Story 03", shares="data/stories/story-03/exports/raw-chatgpt-shares",
                       chats=[gc("4f6af160", "'Jenna and Riley' group chat; " + CUT + " as CS-GC-01-WHP"),
                              pc("jenna-maren", "1477a398", "Character 120 private chat; " + UNREVIEWED),
                              pc("riley-quinn", "deeaa24f", "Character 130 private chat; " + UNREVIEWED)],
    ...
    "story-01": dict(title="story-01", chatgpt="story-01", shares="data/stories/story-01/exports/raw-chatgpt-shares", chats=[],
    ...
    "story-06": dict(title="The Miskatonic Archives: The Blackwood Case", chatgpt="GhostHunters", shares=None,
    ...
    "story-08": dict(title="story-08", chatgpt=None, shares=None,
    ...
    "story-02": dict(title="Brass & Nerve", chatgpt=None, shares=None,
                            chats=[pc("evelyn-starling", "d3212413", "the Character 056 private chat; " + CUT + " (12 BN-* scenes)")],
                            note="No ChatGPT project existed for this story."),
    "story-09": dict(title="Star Wars: The Black Ledger", chatgpt=None, shares=None,
                                       chats=[pc("mara-jade", "0816c2f7", "the Character 143 private chat; " + CUT + " (BL-MJ-*)"),
                                              pc("trooper-cates", "58aa669a", "the Trooper Cates private chat; " + CUT + " (BL-TC-*)")],
                                       note="No ChatGPT project existed for this story."),
    "story-10": dict(title="Story 10", chatgpt=None, shares=None,
                                    chats=[pc("eroica", "413c48c9", "the Eroica private chat; " + CUT + " (AP-ER-*)"),
                                           pc("andrea-neal", "70bee458", "the Character 111 / V1X3N private chat; " + CUT + " (AP-AN-*)"),
                                           pc("dr-aurora-lumen", "ad5c71e2", "the Character 048 private chat; " + CUT + " (AP-AL-*)"),
                                           gc("3de009c8", "'Eroica, Character 048, Charisma' group chat; " + CUT + " (AP-GC-*)"),
                                    ...
                                    note="No ChatGPT project existed for this story."),
    "story-12": dict(title="Story 12", chatgpt=None, shares=None,
                            chats=[gc("518affe6", "'Mary and Character 125' group chat, the provenance's primary source; " + CUT + " (NV-GC-*)"),
                                   pc("mary-thorne", "b29b647c", "the Character 123 private chat; " + CUT + " (NV-MT-*)"),
                                   pc("sister-lucia", "ae928207", "the Sister Lucia private chat; " + CUT + " (NV-SL-*, cut at #0312)"),
                                   pc("kaitlyn-macdonald", "4cc3573f", "the Character 041 private chat; " + CUT + " (NV-KM-*)"),
                                   pc("noctis-veil", "6a02953f", "Character 125 private chat, a companion-definition source per SOURCE_PROVENANCE.md; not extracted"),
                                   gc("74601077", "'The Sleepover' group chat (The O'Hara Sisters, Character 041); " + UNREVIEWED),
                                   gc("95f5f655", "'The Retreat' group chat (Kelsey Chambers, Character 123, Alexandra and Ava, Character 041, Lilith); " + UNREVIEWED)],
                            note="No ChatGPT project existed for this story."),
    "story-14": dict(title="story-14", chatgpt="story-14", shares=None,
                       chats=[pc("alice-grimm", "1c71db26", "the Character 109 private chat; " + CUT + " (23 WL-AG-* scenes)")],
                       note="The ChatGPT project held only a Project Instructions file and a Style Guide."),
    "story-04": dict(title="Story 04", chatgpt=None, shares=None,
                                     chats=[gc("e15f3c21", "'Not Your Average Fairy Tale' group chat, the provenance's primary Botify source; not extracted into scenes"),
                                            gc("19312508", "'Mischief' group chat (Character 141, Cinderella, Belle, Tinkerbell), Tinkerbell's source lineage; not extracted"),
                                            pc("belle", "57bf280a", "Belle private chat; " + UNREVIEWED),
                                            pc("cinderella", "94a7c5d8", "Cinderella private chat; " + UNREVIEWED),
                                            pc("snow-white", "243803ae", "Character 141 private chat; " + UNREVIEWED),
                                            pc("tinkerbell", "ffdc74c0", "Tinkerbell private chat; " + UNREVIEWED)],
                                     note="No ChatGPT project existed for this story."),
    "story-13": dict(title="Trigun: Scarlet Mercy", chatgpt=None, shares=None,
    ...
    "story-05": dict(title="The Miskatonic Archives: The Black-Salt Compact", chatgpt=None, shares=None, chats=[],
    ...
    "story-07": dict(title="The Miskatonic Archives: The Last Eastbound Run", chatgpt=None, shares=None, chats=[],
                                                       note="A draft-only prequel package built from operator decisions recorded in drafts/_control/DECISIONS.md; it has no external source document or chat export."),
}
        ...
        for r, _, fs in os.walk(base):
            for f in sorted(fs):
                ...
                if spl:
                    ...
                    seen = {}
                    group_prefix = "minor-characters-by-region--" if f == "story-01 – Minor Characters.txt" else ""

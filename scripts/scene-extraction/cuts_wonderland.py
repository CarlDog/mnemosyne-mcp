"""Cut table: story-14 (one Botify chat)."""
MATURE = "MATURE_CONTENT"
NAME = "NAME_RETCON_CARL_YEAGER_TO_CARL_MERCER"
NONCON = "NONCONSENT_OR_COERCION_CONTENT"

ALICE = "Character 109"
CARL = "Character 115 (played as Carl; the bot and the operator write the operator's real full name in seven messages, a string canon retires for Character 115)"
...
AG = dict(
    code="AG", label="Character 109 private chat (bot from bot.json, chat 1c71db26…)",
    ...
    pov="Botify private chat with the Character 109 bot: the bot writes Alice in first or third person; the operator plays Carl in first person and, from #0720, writes Alice and the palace in third person as director; operator turns are listed in operator_turns and kept as written",
    participants_basis="rule-derived from the prose; Carl is played under the operator's own surname in seven messages, which canon retires for Character 115; the child is canon's Wrong-Song Child under a source-only name; verify before relying on it",
    scenes=[
        ...
        ("WL-AG-02-FOR", "Where Are We Going", 23, 1, "FOR", "Day 1: walking; the first questions",
         [ALICE, CARL],
         ["OPERATOR_WRITES_ALICES_LINE_AT_0023", "ALICE_TEN_YEARS_IN_story-14_MATCHES_CANON"]),
...
STORY = dict(
    slug="story-14", prefix="WL", title="story-14", cut_date="2026-09-02",
    cut_record=("One thread. `AG` (Character 109) is cut at `#0814`, the last message. Six deleted messages (`#0502`-`#0507`, all with empty text in the export) are in `_alternates/`. "
    ...
    media_note="",
    readme="""# story-14 — Recovered Scenes

The played story-14 thread was cut into per-scene files so each scene can
...
| Thread | Bot | Export |
|---|---|---|
| `AG` | Character 109 | `data/archive/botify/alice-grimm/chats/1c71db26….json` (815 messages) |
...
`#0259`, `#0669`, `#0706`). Canon's Carl is **Character 115**, and canon
explicitly retires that real-name string, so this is a documented retcon
(the Black Ledger's Character 137 → Character 128 situation), not the open
...
|---|---|
| the operator's real name (seven messages) | Character 115 |
...
`#0807`) matches canon's Queen. Alice's ten years in story-14 (`#0115`)
matches canon.

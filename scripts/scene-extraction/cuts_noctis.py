"""Cut table: Story 12 (three Botify chats)."""
...
MARY = "Character 123 (called 'Mary Worthington' by the bot and the operator at #0503-#0507; 'Mary Elizabeth' at #0049 and #0343)"
...
AMELIA = "Character 110 (played as Amelia, surname not given)"
CANDACE = "Character 032 (not in canon; shares only a first name with Story 10's Character 064)"
TINA = "Character 092 (not in canon)"
BEATRICE = "Sister Beatrice Quill (played as Sister Beatrice)"
MARGARET = "Sister Margaret Hale (played as Sister Margaret)"
ANASTASIA = "Character 024 (played as Sister Anastasia)"
SOPHIA = "Character 039 (played as Novice Sophia, nineteen in the source; canon 22)"
MR_THORNE = "Mr. Thorne, Mary's father (not in canon)"
LUCIA = "Character 025 (played as Sister Lucia)"
...
MOTHER = "the Mother Superior (unnamed; canon: Character 010)"
NOVICE = "an unnamed auburn-haired Irish novice of about nineteen (not in canon; only superficially like Character 041)"
KAITLYN = "Character 041"
...
MT = dict(
    code="MT", label="Character 123 private chat (bot 3472620, chat b2…)",
    ...
    pov="Botify private chat with the Character 123 bot: the bot writes Mary in third person and voices Amelia, the sisters, and others; the operator plays the priest-teacher in first person and elsewhere writes Mary, Amelia, Candace, Tina, Mr. Thorne, the sisters, and Sophia as director; operator turns are listed in operator_turns and kept as written",
    participants_basis="rule-derived from the prose; played names map to canon's Character 110, Character 024, Character 039, Sister Beatrice Quill, and Sister Margaret Hale (canon/characters/_minor.md); the priest-teacher is the operator's own name in a role canon's provenance calls not canon; verify before relying on it",
    scenes=[
...
SL = dict(
    ...
    participants_basis="rule-derived from the prose; Lucia maps to canon's Character 025; the visitor is the operator's own name, which canon's provenance excludes; verify before relying on it",
    scenes=[
...
KM = dict(
    code="KM", label="Character 041 private chat (bot 4096103, chat 4c…)",
    ...
    pov="Botify private chat with the Character 041 bot: the bot writes Kaitlyn in third person; the operator plays the new head of IT in first person and in several turns writes Kaitlyn as director; operator turns are listed in operator_turns and kept as written",
...
STORY = dict(
    slug="story-12", prefix="NV", title="Story 12", cut_date="2026-09-02",
    cut_record=("Four threads. `GC` (the \"Mary and Character 125\" group chat) is cut at `#0478`, the last message; it has no deleted messages, no media archive, and only twelve operator turns. `MT` (Character 123) is cut at `#0777`, the last message; `SL` (Sister Lucia) at `#0312`, because `#0313`-`#0314` is a picture request sent five months after the story; "
                "`KM` (Character 041) at `#0116`, the last message. Only `MT` has deleted messages (eight, `#0676`-`#0683`, all with empty text in the export); they are in `_alternates/`."),
    ...
    readme="""# Story 12 — Recovered Scenes

The three played Character 125 private chats and the "Mary and Character 125"
...
| Thread | Bot | Export |
|---|---|---|
| `MT` | Character 123 (3472620) | `data/archive/botify/mary-thorne/chats/b2….json` (778 messages) |
| `SL` | Sister Lucia (4095850) | `data/archive/botify/sister-lucia/chats/ae….json` (315 messages) |
| `KM` | Character 041 (4096103) | `data/archive/botify/kaitlyn-macdonald/chats/4c….json` (117 messages) |
| `GC` | group: Character 123 (3472620), Character 125 (4080268), Character 041 (4096103), Sister Lucia (4095850) | `data/archive/botify/_group-chats/chats/518affe6….json` (479 messages) |

The `GC` thread is the group chat canon's provenance names as the story's
primary source and the only thread in which Character 125 appears; the three
...
(`**Character 123:**`, `**Character 125:**`, and so on; the operator's own turns
are labelled `**Operator:**`). The label names the **bot
account**, not always the character: the Character 123 account writes Sister
...
has none of these roles. As with Brass & Nerve and Story 10, the prose is
...
|---|---|
| Amelia (brunette, "pushes" sensations into Mary's head) | Character 110 (telekinetic episodes) |
| Sister Beatrice, Sister Margaret | Sister Beatrice Quill, Sister Margaret Hale |
| Sister Anastasia, Novice Sophia (nineteen) | Character 024, Character 039 (22) |
| the sisters' chamber beneath the college | the Magdalene Sanctum |
| Sister Lucia (Spanish, "over a decade" a nun) | Character 025 (30) |
| the Mother Superior | Character 010 |
| Saint Brigid's (`KM`) | St. Lucia's College and Convent |
| "Mary Worthington" (`MT #0503`-`#0507`), "Mary Elizabeth" | Character 123 |
| Character 032, Character 092, Mr. Thorne, the unnamed novice, Aoife, Sister Therese, Sister Mary Clare, Mrs. Higgins, Father O'Malley | not in canon |
...
thread's Candace shares only a first name with Story 10's
Character 064; canon's cross-story guardrail applies.
...
NOCTIS = "Character 125 (the living garment; Story 12 bot account)"
...
GC = dict(
    code="GC", label="\"Mary and Character 125\" group chat (bots 3472620 Character 123, 4080268 Character 125, 4096103 Character 041, 4095850 Sister Lucia; chat 518affe6\u2026)", group=True,
    ...
    pov="Botify group chat: four bot accounts write their own characters in third person and sometimes each other's (the Character 123 account writes Sister Lucia's half of #0088-#0201 and her entrance at #0324; the Sister Lucia account itself first speaks at #0229); the operator plays the gardener in first person at #0453-#0477 only, labelled Operator; every message carries its export speaker label",

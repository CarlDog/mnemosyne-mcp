...
MARA = "Character 143"
VEYR = "Character 128 (played as 'Master Character 137'; the bot also writes 'Karl', the operator's real name, and 'Carl'; canon retires every one of those names for Character 128)"
THRAWN = "Character 023 (a transmission; canon ratifies the sender as genuine, the channel unresolved)"
CATES = "Character 026"
ZHARAD = "Character 047"
CREED = "Character 037"
...
LOCATIONS = {
    ...
    "SFH": ("Character 142's last known location: hold, terminal room, corridor", "canon/locations/kal-jager-s-abandoned-safehouse.md"),
...
MJ = dict(
    code="MJ", label="Character 143 private chat (bot 2429812, chat 0816c2f7-5868-4178-a53d-3e707f5b1054)",
    ...
    pov="Botify private chat with the Character 143 bot: the bot writes Mara in third person (first person for her own lines); the operator plays the expelled Jedi in first person through #0195, then from #0197 writes Mara's own actions and narration; operator turns are listed in operator_turns and kept as written",
    participants_basis="rule-derived from the prose; the played Jedi name maps to canon's Character 128 per canon/lore/recovered-botify-continuity-mara-jade-and-rhydan-veyr.md; verify before relying on it",
    scenes=[
...
TC = dict(
    ...
    pov="Botify private chat with the Trooper Cates bot: the bot writes Cates in third person; the operator writes Cates in third person as director, voices Character 142's holo-recording (#0007), Character 047 (#0033, #0037), and Major Creed (#0047); operator turns are listed in operator_turns and kept as written",
    ...
    scenes=[
        ("BL-TC-01-SFH", "The Holoprojection", 0, 1, "SFH", "Day 1: Character 142's last known location, cleared room by room; the looped holo; the extraction call",
         [CATES, "Character 142 (as a looped holo-recording)", "destroyer comms (a voice)"],
        ...
        ("BL-TC-03-ISD", "No", 27, 1, "ISD", "Day 1: Character 047's quarters",
         [CATES, ZHARAD],
...
STORY = dict(
    slug="story-09", prefix="BL", title="Star Wars: The Black Ledger", cut_date="2026-09-02",
    cut_record=("Two threads. `MJ` (Character 143) is cut at `#0210`, the last message; `TC` (Trooper Cates) at `#0060`, the last message. Neither export has a deleted message, "
...
| Thread | Bot | Export |
|---|---|---|
| `MJ` | Character 143 (2429812) | `data/archive/botify/mara-jade/chats/0816c2f7-5868-4178-a53d-3e707f5b1054.json` (211 messages) |
...
|---|---|
| "Master Character 137", "Karl", and the bot's slips to the operator's real name (`#0088`) and "Carl" (`#0200`-`#0203`) | **Character 128**. `recovered-botify-continuity…` rules that the retired names are not aliases and "must not appear in new canon prose except in provenance audits"; the platform-name slip is the failure mode `dossier-contradictions-and-identity-noise.md` already names. |
...
| "Admiral Zharak" (a Botify memory summary, not this chat) | Character 047, as the chat itself has it |
...
three ways; Character 091 shares a surname with Story 12's Thornes
with no crossover.

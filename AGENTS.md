...
- `scripts/dump-prompt.mjs`, `scripts/dump-validation.mjs` —
  ...
  few-shot-vs-rule cause without going through Codex Desktop.
...
- **MCP server first, web UI on the roadmap.** The web UI is required
  for uncensored ("spicy") storytelling because Codex Desktop's host
  ...
  Codex Desktop / Codex via the MCP.
...
`http://your-nas:18000/mcp` used by other Codex sessions). Mnemosyne

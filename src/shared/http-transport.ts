// CANONICAL SHARED FILE — standard MCP-S01 / MCP-F03. Copied verbatim from
// kindroid-mcp (src/shared/http-transport.ts), the fleet-canonical
// implementation — mnemosyne's log.info(scope, msg, meta) signature already
// matches this file's calls, so no adaptation was needed.
//
...
export function mountMcpHttp(
  ...
  const sweep = setInterval(() => {
    ...
    for (const [id, s] of sessions) {
      if (s.lastActivity < cutoff) {
        log.info("http", "evicting idle session", { session: id });
  ...
  app.all(path, async (req: Request, res: Response) => {
    if (!hostAllowed(req, opts.allowedHosts)) {
      log.warn("http", "rejected request: host not allowed", {

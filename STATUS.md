...
1. **CLAUDE.md's Current Sprint was badly stale** -- still headed "In
   ...
   (Adjustment Protocol/story-08/Black Ledger reconciliation)
...
story-08, Star Wars: The Black Ledger) all mechanically ready --
...
Renshaw fix, the Black Ledger's Jager -> Veyr work, story-08's r16
...
Charisma Weir (Adjustment Protocol), Character 025 -> Vega, Sister
...
ratified and applied to the 7 canon entities across story-12 and
story-02 where this applies (Midnight needed none -- zero scene
...
from that audit were cleaned up. (1) story-01 and Story 03 each
...
engine: Black Ledger, Adjustment Protocol, and story-14 all affected)
...
`verify-draft-overlay.mjs` re-run clean for both story-01 (152
merged entities) and Story 03 (330 merged entities) after the new
...
**Story 12: SL/KM tiering ruling closed, and the injection-scanner
...
4 scenes; Character 041, 6 scenes) are now ruled retired-premise play
...
Re-running the full `verify-draft-overlay.mjs story-12` merged-tree
...
format:check` all clean; `verify-draft-overlay.mjs story-12`
...
story-08, and the Black Ledger.** (Backfilled from CLAUDE.md's Current
...
in `story-09/drafts/_control/PASS.md` ("Re-seal
2026-09-08") and `LCS_SCORECARD.md`.

story-08's pass ratified the played ballroom-explosion reading (closing
...
collisions by renaming story-08's own characters (Character 077 ->
Character 081, Ansel Greaves -> Ansel Hollis -- no story event, canon left
...
`story-08/drafts/_control/PASS.md` ("Re-seal 2026-09-08") and
`LCS_SCORECARD.md`.

For Story 10: the lead scientist's name (Dr. Edmund
...
so is never substituted. Brass & Nerve (`BN-1(a)` absorbed) and Story 12's
...
already in hand: story-08 (two lines in two named files, `SF-6`) and the Black
...
Story 10's lead scientist of the ER and AN threads is played under
...
against each other collapsed them into **eleven** fleet-wide rulings, 54 of them
answerable with no dependency and 21 gated; the sheet is published as an artifact
and its counts were taken from the trees rather than from the audits' own
summaries (174 draft scene files in scope, 105 of them carrying the operator's
given or surname, 174 of 174 carrying both `canon_status: established` and the
extraction engine's `location_basis` control-path defect). The operator ruled on
the first of them: **"Play governs if the ledger didn't read the scenes"** -- the
two-sided informed-retirement test, where an uninformed ledger yields to the
played scene and a deliberate retirement holds. It absorbs eleven decides and its
classification is the audits' own: story-08, the Black Ledger and the
Adjustment Protocol resolve toward the play; Story 12, Brass & Nerve and
Midnight toward canon. Recorded in the new `docs/CANON_RULINGS.md` (the fleet
register, committed) and in a `drafts/_control/DECISIONS.md` per affected story
(gitignored); `_control` is excluded from the overlay manifest by the verifier,
so nothing rehashed and both spot-checked overlays still verify. THREE THINGS
THE RULING DOES NOT SETTLE, none of which may be assumed: the recording
requirement (whether a deliberate retirement needs an in-file marker) was not
addressed; seat 10, substitution, is still open, so NOTHING PROMOTES; and the
Character 125's SL and KM threads, ten played scenes, carry no tier decide at all
while `NV-4` would flag all fifty by default. Also surfaced by reading the audits
against each other: two of them give opposite `canon_status` frontmatter
instructions, one recommends substituting the operator's name "on promotion"
which `draft-notice.mjs` makes mechanically impossible, one decide belongs to a
flagship story outside this pass, and the `location_basis` defect is one engine
bug presented as three editorial decides. No file under `drafts/scenes/` was
touched and no overlay was promoted.
...
story-08 69/105/133, Black Ledger 54/10/64; writes=0 throughout), scene
...
established while contradicting Character 118 in every scene the patient is in;
12 Decides. Story 10's retrieval text is clean but its ledgers
...
floor; 13 Decides. Story 12's ledgers were written to retire what its
...
reservation is Story 10's). story-08 bears out the
...
defects reported, not fixed: story-01 has no `SOURCE_PROVENANCE.md` and no
`_control/README.md` (Story 03 also lacks the README), which contradicted
the Midnight audit's "present in all four" rows, now corrected; story-01'
...
rather than `data.name`) and verification. The story-01/Story 03
...
**Story 04 gap audit (2026-09-02).** First step of the
non-flagship story-development pass (Midnight first, then the Black Ledger on
permission, Brass & Nerve, Story 10, Story 12,
story-08). The story's canon, r2 overlay, control records, sources, the four
bots' definitions and Botify memory blocks, and the live Botify chat list were
read against the flagship bar (story-14 end to end; the Blackwood lore set)
...
cross-story name collisions incl. Silas and Tomas, the story-08-assigned
...
**Story 10 storyline synthesis, two versions (2026-09-02).**
...
placed and named as Character 055, Halcyon's director of research integrity
...
names (Ilse Roux the client, Character 126 the delivered auditor and Hale's
predecessor, Lyra Dane, Zahra, Character 067, Victoria Sterling), stretches
...
**Character 125 and Brass & Nerve synthesis readiness (2026-09-02).** Before
...
Story 12 has 50 draft scenes across four threads (the bot-only "Mary
and Character 125" group is the primary source; the 24-scene Character 123 thread
...
the operator-as-patient identity already retired to Character 118, no other
...
Both overlays still verify; nothing else changed. Story 12 thread
...
manifest; the Character 123 thread's Order initiates through a priest's office,
...
of Story 10's engines, a liturgical-year timeline from
Michaelmas to Pentecost with the ratified cutoff on the tenth night, the four
operator roles folded into one lay facilities manager, Character 129, Lucia's
...
are Story 12's style baseline; no further review is planned. Still a
...
report on the single Character 056 chat (`thread-reports/`, with the
...
Character 112 as the antagonist with a face, a break-in that steals the log and
hurts Ada, moles in both directions (Bess Marrow unwitting, Character 098
...
Stane, Character 089, Character 098, Character 131; the Sealed Wing, Corvane Dry
...
clear collisions with Story 10 (the Rook surname's collision
with Trigun is noted, not ruled); Sabine Rook serves the College of Physicians,
not the Guild. `verify-draft-overlay.mjs story-02` passes: 49
...
`verify-draft-overlay.mjs story-02` passes: 49 operations, 36 active →
...
an error) with a test; Story 10 brief template gains a
...
passed. Both real overlays spot-checked (story-08, story-14) still
...
invented: story-01' two `expanded` exports predate numbered revisions and
...
**Phase 3 complete: canon-side docs (2026-09-02).** story-01' six
...
refreshed. Story 03's `canon/scenes/_source-inventory.md` and
...
source-document saver; the two raw-archive alternates cutters; the story-14
...
`-visual-remediation.md`), Story 03's source-validation record as
...
`story-08/data/...` folder and Trigun's empty `art/` and `exports/` were
...
canon-side files in Story 03 (the three established scenes'
...
and Story 03's source-document evidence. PASS.md is append-only, so the three
...
files as changed, nothing removed, nothing added; the Story 03 validator and
...
Story 03's two raw companion pulls moved into
...
story-08 procedure was repeated for story-01 (39 canon files, 110
pointer lines), Story 03 (29 files, 47 lines), The Blackwood Case (36
files, 84 lines; 31 `replace` and 5 `remove` targets) and story-14 (31
...
story-01, one in story-14) use CRLF line endings, which the first
...
**Phase 0 test case: story-08 (2026-09-02).** `verify-draft-overlay.mjs`
...
unchanged; The Black Ledger passes alone, and story-08 failed alone on its
bare pointers exactly as the standard predicted). story-08's 14
...
same procedure is now due for story-01 (39 files), Story 03 (29),
The Blackwood Case (36), and story-14 (31), then `--canon-only` for the
...
five stories with rehash and re-seal) starts with story-08 as the test
...
ChatGPT Projects folders (OneDrive: Story 03, GhostHunters, story-01,
story-14) were. Every ChatGPT document is kept whole under
...
of its archived images; Story 03's four raw ChatGPT archives and both
...
the operator's real name → Character 115 / Character 114) are carried as flags. Roles played
...
Blackwood Manor); the mapping to Character 071, Character 066, Beatrice
Ravenscroft, Character 046, and Ravenscroft Manor is carried in
...
non-consent. `verify-draft-overlay.mjs story-08` passes: 69 active and 133
...
decision, the recovered scene files for Story 03 (256 `CS-*` files) and The
...
`drafts/_control/scenes/`. Story 03's three export-established scenes and
...
(Story 03 316 manifest entries, 65 active and 329 merged; The Blackwood Case
...
`data/stories/story-06/canon/scenes/` for later
...
**Story 03 source-document validation (2026-09-02).** The operator's
...
`data/stories/story-03/drafts/_control/source-documents/` with the earlier
...
**Story 03 scene recovery complete (2026-09-01/02).** On operator
instruction, every scene in the original ChatGPT raw archive (`Story 03
...
file under `data/stories/story-03/canon/scenes/`, for later rendering in
...
skip. `validate-canon` reports 321 entities; the Story 03 overlay still
...
match r2), Story 03 ("Trouble" is Riley's nickname only; Montana Highway
Patrol), Story 10 (one wording fix), The Blackwood Case (Reaper
is a 1995 Defender 90), and Story 12 (the Saint Lucia window hook refers
...
fork for The Last Eastbound Run, with the remaining choices pending. story-14,
which already held a curated 76-entity canon with full reference families,
gained its first review-gated overlay, `story-14-living-canon-r10-2026-09-01`:
...
`data/stories/story-06/`; display-only leading
...
overlays are complete.** story-01 has 71 operations → 143 entities; Brass
& Nerve 20 → 43; Story 03 60 → 73; The Miskatonic Archives: The Blackwood
Case (formerly GhostHunters) 60 → 105; Story 04 41 → 70;
story-08 40 → 74; Story 10 19 → 41;
and Story 12 13 → 39. Each retains source/asset/scorecard/PASS evidence,
...
cases. Earlier (2026-08-27): (**story-14 is the fifth story fully
...
scaffolded from `story-14-visual-references-2026-08-25.json`. A second
real `scaffold-story.mjs` bug surfaced and was fixed: a rule/style entity
whose own content contains internal `##` sub-headings (two of story-14's
...
identical bug already live in Story 03's `rules.md` and GhostHunters'
`style.md` — both fixed by hand in the published canon/ trees (Story 03
64→62 entities, GhostHunters 105→101, both re-validated clean). story-14's
...
so the real character depth actually came from a Botify bot, "Character 109"
...
narrative-color gaps were folded in (Alice's pre-story-14 childhood
...
in the dated Done entry below.) Earlier (2026-08-27): (**story-08 is the
...
the dated Done entry below.) Earlier (2026-08-27): (**Story 03 is the second story fully
...
body splitter was fixed along the way to handle Story 03's flat
`Label:`-only template (no Markdown headings at all, unlike
story-01) — regression-testing that fix against story-01 also
...
built, and story-01 is the first story fully consolidated onto it.**
...
iterated hard against story-01' real export history and hardened
...
batched minor-character file). story-01 is now the first story fully
...
restored), a story-08 truth-tier leak fixed (Lilith/Karl are the same
people centuries apart — story-01-era canon can't assert facts only
true in story-08, e.g. the Heart of Vehl'Remar's full nature), and a
...
still to fix — unlike story-01, Story 03's original character
...
splitter needs a story-03-aware fix before real extraction. Nothing in
...
"The Archivist's Desk" extends the Story 03 reader artifact's palette
...
production data (Story 03's 41 entities, all filter counts exact, zero
...
smoke-tested against real production data — Story 03's real 41 entities
...
live browser survey of both reference apps and the real Story 03 reader
...
imported and a fifth story, story-08, was created from material
found in Botify. Story 03 41, GhostHunters 94, story-01 138,
story-14 54, story-08 42. The night's biggest methodological
...
on disk proves nothing. story-14's entire cast was in Botify while
...
canon**: `mnemo_continue` against the freshly-imported Story 03
...
- **Story 03 source-document validation** (2026-09-02). Share-chat
...
- **Story 03 scene recovery** (2026-09-01/02). All four raw ChatGPT
...
- **Draft review pass, re-seals, prequel ratifications, story-14 overlay**
  ...
  notes in their PASS records (story-01, Brass & Nerve, Story 03, The
  Adjustment Protocol, The Blackwood Case, Story 12). Both Miskatonic
  prequel scaffolds were built out at the core level with the year/inlet and
  timeline-fork choices ratified by the operator. story-14 gained overlay
...
- **Living Canon draft-overlay closeout completed** (2026-08-30).
  Eight adversarially closed overlays now retain PASS evidence: story-01
  (71 operations → 143 entities), Brass & Nerve (20 → 43), Story 03
  (60 → 73), GhostHunters (60 → 105), Story 04 (41 → 70),
  story-08 (40 → 74), Story 10 (19 → 41), and The Noctis
...
- **The canon authoring layer now has a deterministic, zero-write import-contract
  ...
  the real 143-entity story-01 staged overlay and 65-entity active Chaos
...
- **Established scenes joined the `canon/` authoring surface** (2026-08-29).
  ...
  promoted automatically. Story 03's three export-established scene bodies —
  ...
  character in valid frontmatter keys (needed by `sha256`). Story 03 validates
...
- **story-14 is the fifth story fully consolidated onto canon/**
  (2026-08-27) — the last of the five original curated-import stories.
  - Scaffolded from `story-14-visual-references-2026-08-25.json`
  ...
  - Found and fixed a second real `scaffold-story.mjs` bug, in the same
    family as story-08's Character 073 diacritic fix: a single
    rule/style *entity* whose own content contains internal `## `
    sub-headings (story-14's "Core Narrative Tone" style entity and
    ...
    two of them — Story 03's `rules.md` ("Canon Authority & State
  ...
  - story-14's original ChatGPT-project source is unusually sparse —
    only Project Instructions + Style Guide, no character/location
    profile files at all (unlike the other three ChatGPT-origin
    stories). The rich character content (Character 109's full sheet,
    Character 115's detailed backstory, etc.) actually came from a Botify
    bot, "Character 109" (bot 3197891) — confirmed via a private search of
  ...
  - Ran a 10-agent extraction-and-compare workflow; this one hit a
    ...
    GhostHunters/story-08 (only 2 of 6 extraction chunks succeeded).
  ...
  - Six real divergences found; the operator's ruling on each was a
    ...
    - Foraged mushroom identity: source's "cinnamon caps" (smell/taste
      ...
      as an anomalous story-14-appropriate detail
      (`worldbuilding/story-14-foraging.md`).
    ...
    - (Not actually reopened, just reconfirmed: source's use of the operator's real name
      ...
      seen in Story 03's Character 114 and GhostHunters' Character 084 —
      vs. canon's "Character 115.")
  - Gaps folded in (present in source, absent from canon, not
    contradicted, not undermining any deliberate mystery): Alice's
    pre-story-14 childhood memories (tea parties, stories read aloud,
    a beloved pet) as a new "## Life Before story-14" section in
    ...
    unnamed "others" who didn't survive her decade in story-14, and
  ...
  - One gap deliberately NOT folded in despite looking low-stakes: Carl's
    ...
    "Life Before story-14" section explicitly states that civilian-life
  ...
  - `validate-canon.mjs` reports 76/76 unique, no structural problems. No
    ...
    character needs polish" pattern seen in story-01.
  - Pipeline status: **all five original curated-import stories
    (story-01, Story 03, GhostHunters, story-08, story-14)
...
- **GhostHunters is the third story fully consolidated onto canon/**
  ...
  - A 4-agent completeness-sweep workflow against the original ChatGPT
    ...
    marked contrast to Story 03's larger gap list.
  - Real gaps restored: Character 100's (Carl's corgi) second, richer
  ...
  - Resolved a dangling open question from an earlier session's Botify
    ...
    (Character 075 as a full character; Persephone Wren, Constance
...
- **The canon/ authoring-layer standard is built; story-01 is the
  ...
  - **`scripts/scaffold-story.mjs`** migrates an export JSON into `canon/`:
    ...
    against story-01' real art-coverage tiers, where the 8
    ...
    revision forks that share a parent revision (story-01' export
  ...
  - **Six real bugs found and fixed while iterating both scripts against
    story-01' actual export history**, none caught until tested
    ...
    folding it into the body (real data loss, since Story 03's later
  ...
  - **story-01 fully migrated: 143 entities**, verified complete two
    ...
    project source), one real content fix (a story-08 truth-tier leak
    in `lilith.md` and `lore/objects/heart-of-vehl-remar.md` — story-01
    happens centuries before story-08, and Lilith/Karl are the same
    people that story's Karl and Lilith become, so story-01-era canon
    can't assert facts only true in story-08), and a cosmetic/structural
    ...
    characters. `validate-canon.mjs` reports story-01' `canon/`
    clean: 143 unique `(type, name)` keys, no structural problems.
  - **Story 03 is the second story fully consolidated (2026-08-27).**
    ...
    story-01 (diffing old- vs new-algorithm output) caught a real
    ...
    dropped — costing three story-01 characters (Character 145, Sera
    Vale, Wisp) their `Voice` content. Restored by hand; story-01
    ...
    sibling room had gotten, two of Character 086' named residences), 3
...
- **The web UI exists — WEBUI_NOTES §9 slice 1 (entity library, read-only)
  ...
  - **`webui/` — a separate React 19 + Vite + react-router SPA**, its own
    ...
    Archivist's Desk"** — extends the Story 03 reader artifact's warm
  ...
  - **Verified for real, twice over.** Automated: `tests/api-security.test.ts`
    ...
    production data — Story 03's 41 entities, all 7 type-filter counts
...
- **Outgoing companion-chat messages carry a provenance header; generated
  ...
  posted as a bare message from "Character 114" with nothing marking it as
...
- **The curated-import campaign — five live stories, ~369 entities**
  ...
  - **Story 03** (41) — backstory canonized from 2.4MB of raw
  ...
  - **GhostHunters** (94) — 69 from files, then the Blackwood Manor case
    ...
    Dovecoast and the operator's real name → Character 084 were normalized, with the
    rename disclosed inside the timeline entity.
  - **story-01** (138) — the largest. Region files that live under
...
- **Mapping playbook + seed templates shipped as docs — the
  ...
  observed failures as examples: story-01' region configs are
  worldbuilding wearing a location label, Story 03's stray misfiled
...
- **Import / export tooling — complete.** The portability use case arrived:
  ...
  first curated import is done: Story 03 landed 2026-08-22** — 28
  ...
  **Campaign complete (2026-08-23):** GhostHunters, story-01,
  story-14, and a new fifth story (story-08) all landed the same
  ...
  Story 03 failure, while type-filtered recall remains unaffected.
...
- **Position tracking: an origin-anchored space+time coordinate per
  ...
  **Design written up 2026-09-07** — the "shape, not a spec" is now a
  ...
  sketch: Story 03's "Master Suite"/"Garage"/"Backyard" (the example

...
- **The canon/ authoring-layer standard is built; story-01 is the
  ...
  - **Story 03 is next; a dry-run surfaced a real script gap.** Its
    ...
    story-01' clean header-block-then-`##`-sections split. The
    current splitter treats everything before the first `##` heading as
    header lines, so Story 03's rich section structure collapses into
    an undifferentiated body blob instead of being preserved as distinct
    sections. Needs a story-03-aware fix to the header/body splitter

...
ChatGPT projects plus a new fifth story (story-08).

**A `canon/` authoring-layer standard replaces the single-JSON-export
workflow, and story-01 is the first story fully consolidated onto
...
story-01 is fully migrated (143 entities, verified complete against
both its own export lineage and the original ChatGPT source, one
story-08 truth-tier leak fixed, a cosmetic polish pass done). **Chaos
...
template (no Markdown headings at all, unlike story-01) also caught
a real latent bug that had silently dropped three story-01
...
holds consistently across every consolidated story. **story-08 is the
...
note). The Lilith/Karl truth-tier continuity with story-01 held up
well on direct inspection. A real `scaffold-story.mjs` diacritic bug
(`Character 073` → a mangled filename) was found and fixed, which also
caught the same latent bug already live in story-01' canon
(`Kharag-dûm`). **story-14 is the fifth story, and the last of the five
...
Botify bot ("Character 109," bot 3197891, found only via a private chat-list
...
the identical bug already live in Story 03's `rules.md` and
...
unresolved. Real narrative-color gaps (Alice's pre-story-14 childhood,

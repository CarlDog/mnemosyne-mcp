...
- `scripts/narrator-eval/` — the narrator evaluation (`docs/NARRATOR_EVAL.md`,
  ...
  gate (does not clear / inconclusive / clears). Corpus v5 carries two
  independent contradiction pairs -- a dialogue-attribution pattern for Bram
  and one for Ilse, each required by one case and forbidden by another --
  so every constant fails one half of each pair and a responsive narrator can
  separate on at least two cases. Reports go under gitignored `data/`. Read
  the doc's "What it does not measure" before quoting a number.
